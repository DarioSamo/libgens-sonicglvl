//
// Generated by Microsoft (R) HLSL Shader Compiler 9.29.952.3111
//
// Parameters:
//
//   float4 g_Ambient;
//   float4 g_Diffuse;
//   float4 g_EyeDirection;
//   float4 g_EyePosition;
//   float4 g_ForceAlphaColor;
//   float4 g_GI0Scale;
//   float4 g_GI1Scale;
//   sampler2D g_GISampler;
//   float4 g_LightScatteringColor;
//   float4 g_OpacityReflectionRefractionSpectype;
//   float4 g_PowerGlossLevel;
//   float4 g_ShadowMapParams;
//   sampler2D g_ShadowMapSampler;
//   float4 g_Specular;
//   float4 g_ViewportSize;
//   float4 mrgEdgeEmissionParam;
//   float4 mrgEyeLight_Attribute;
//   float4 mrgEyeLight_Diffuse;
//   float4 mrgEyeLight_Range;
//   float4 mrgEyeLight_Specular;
//   float4 mrgGlobalLight_Diffuse;
//   float4 mrgGlobalLight_Direction;
//   float4 mrgGlobalLight_Specular;
//   float4 mrgLocalLight0_Color;
//   float4 mrgLocalLight0_Position;
//   float4 mrgLocalLight0_Range;
//   float4 mrgLocalLight1_Color;
//   float4 mrgLocalLight1_Position;
//   float4 mrgLocalLight1_Range;
//   float4 mrgLocalLight2_Color;
//   float4 mrgLocalLight2_Position;
//   float4 mrgLocalLight2_Range;
//   float4 mrgLocalLight3_Color;
//   float4 mrgLocalLight3_Position;
//   float4 mrgLocalLight3_Range;
//   float4 mrgPlayableParam;
//   float4 mrgTexcoordIndex[4];
//   sampler2D sampDif;
//   sampler2D sampPow;
//
//
// Registers:
//
//   Name                                  Reg   Size
//   ------------------------------------- ----- ----
//   mrgGlobalLight_Direction              c10      1
//   g_Diffuse                             c16      1
//   g_Ambient                             c17      1
//   g_Specular                            c18      1
//   g_PowerGlossLevel                     c20      1
//   g_OpacityReflectionRefractionSpectype c21      1
//   g_EyePosition                         c22      1
//   g_EyeDirection                        c23      1
//   g_ViewportSize                        c24      1
//   mrgTexcoordIndex                      c26      1
//   mrgGlobalLight_Diffuse                c36      1
//   mrgGlobalLight_Specular               c37      1
//   mrgLocalLight0_Position               c38      1
//   mrgLocalLight0_Color                  c39      1
//   mrgLocalLight0_Range                  c40      1
//   mrgLocalLight1_Position               c42      1
//   mrgLocalLight1_Color                  c43      1
//   mrgLocalLight1_Range                  c44      1
//   mrgLocalLight2_Position               c46      1
//   mrgLocalLight2_Color                  c47      1
//   mrgLocalLight2_Range                  c48      1
//   mrgLocalLight3_Position               c50      1
//   mrgLocalLight3_Color                  c51      1
//   mrgLocalLight3_Range                  c52      1
//   mrgEyeLight_Diffuse                   c58      1
//   mrgEyeLight_Specular                  c59      1
//   mrgEyeLight_Range                     c60      1
//   mrgEyeLight_Attribute                 c61      1
//   g_ShadowMapParams                     c65      1
//   g_GI0Scale                            c70      1
//   g_GI1Scale                            c71      1
//   g_LightScatteringColor                c75      1
//   mrgPlayableParam                      c86      1
//   mrgEdgeEmissionParam                  c88      1
//   g_ForceAlphaColor                     c89      1
//   sampDif                               s0       1
//   sampPow                               s2       1
//   g_GISampler                           s10      1
//   g_ShadowMapSampler                    s13      1
//

    ps_3_0
    def c0, 0.5, 0.298900008, 0.586600006, 0.114399999
    def c1, 2, -1, -2, 3
    def c2, 500, 5, 1024, 4
    def c3, 0, 1, 2, 3
    dcl_texcoord v0
    dcl_texcoord1 v1
    dcl_texcoord2 v2.xyz
    dcl_texcoord3_pp v3
    dcl_texcoord4 v4
    dcl_texcoord5_pp v5.xyz
    dcl_color_pp v6
    dcl vPos.y
    dcl_2d s0
    dcl_2d s2
    dcl_2d s10
    dcl_2d s13
    rcp r0.x, v4.w
    mul r0.xy, r0.x, v4
    mad r0.xy, r0, c1.x, c1.y
    abs_sat r0.xy, r0
    add_sat r0.xy, r0, -c65.x
    mul r0.xy, r0, c65.y
    max r1.x, r0.x, r0.y
    max r0.x, v4.z, c3.x
    add r0.y, r0.x, -v4.w
    cmp r0.z, r0.y, c3.x, r0.x
    mov r0.xyw, v4
    texldp_pp r0, r0, s13
    lrp_pp r1.y, v5.z, c3.y, r0.x
    lrp_pp r0.x, r1.x, c3.y, r1.y
    texld_pp r1, v0.zwzw, s10
    mul_pp r0.yzw, r1.xxyz, r1.xxyz
    dp3_pp r0.y, r0.yzww, c0.yzww
    mad_pp r1.xyz, r1, r1, -r0.y
    mad_sat_pp r0.yzw, r1.xxyz, c71.w, r0.y
    mul_pp r0.yzw, r0, c70.xxyz
    mul_pp r1.x, r0.x, r1.w
    mul_pp r1.yzw, r0, c17.xxyz
    mul r1.yzw, r1, c70.w
    mad r0.yzw, r0, c17.xxyz, -r1
    mad_pp r0.xyz, r0.x, r0.yzww, r1.yzww
    add_pp r0.w, -c61.z, c61.y
    rcp_pp r0.w, r0.w
    add r1.yzw, -c22.xxyz, v2.xxyz
    dp3 r2.x, r1.yzww, r1.yzww
    rsq r2.x, r2.x
    mul_pp r1.yzw, r1, r2.x
    rcp r2.x, r2.x
    add r2.x, r2.x, -c60.z
    dp3_pp r2.y, r1.yzww, c23
    add_pp r2.y, r2.y, -c61.z
    mul_sat_pp r0.w, r0.w, r2.y
    mad_pp r2.y, r0.w, c1.z, c1.w
    mul_pp r0.w, r0.w, r0.w
    mul_pp r0.w, r0.w, r2.y
    pow_sat_pp r2.y, r0.w, c61.w
    mov r3.xy, c3
    add r0.w, r3.y, -c61.x
    cmp_pp r0.w, r0.w, c3.y, r2.y
    add r2.y, -c60.z, c60.w
    rcp r2.y, r2.y
    mul_sat r2.x, r2.y, r2.x
    add_pp r2.x, -r2.x, c3.y
    cmp_pp r2.x, -c61.x, r3.y, r2.x
    mul_pp r0.w, r0.w, r2.x
    add r2.x, -c48.z, c48.w
    rcp r2.x, r2.x
    add r2.yzw, -c46.xxyz, v2.xxyz
    dp3 r3.y, r2.yzww, r2.yzww
    rsq r3.y, r3.y
    rcp r3.z, r3.y
    mul_pp r2.yzw, r2, r3.y
    add r3.y, r3.z, -c48.z
    mul_sat r2.x, r2.x, r3.y
    add_pp r2.x, -r2.x, c3.y
    add r3.y, -c40.z, c40.w
    rcp r3.y, r3.y
    add r4.xyz, -c38, v2
    dp3 r3.z, r4, r4
    rsq r3.z, r3.z
    rcp r3.w, r3.z
    mul_pp r4.xyz, r3.z, r4
    add r3.z, r3.w, -c40.z
    mul_sat r3.y, r3.y, r3.z
    add r3.z, -c44.z, c44.w
    rcp r3.z, r3.z
    add r5.xyz, -c42, v2
    dp3 r3.w, r5, r5
    rsq r3.w, r3.w
    rcp r4.w, r3.w
    mul_pp r5.xyz, r3.w, r5
    add r3.w, r4.w, -c44.z
    mul_sat r3.z, r3.z, r3.w
    add_pp r3.yz, -r3, c3.y
    nrm_pp r6.xyz, v3
    dp3_pp r3.w, r6, -r5
    mul_pp r7.xyz, r3.w, c43
    cmp_pp r7.xyz, r3.w, r7, c3.x
    mul_pp r7.xyz, r3.z, r7
    dp3_pp r3.w, r6, -r4
    mul_pp r8.xyz, r3.w, c39
    cmp_pp r8.xyz, r3.w, r8, c3.x
    mad_pp r7.xyz, r3.y, r8, r7
    dp3_pp r3.w, r6, -r2.yzww
    mul_pp r8.xyz, r3.w, c47
    cmp_pp r8.xyz, r3.w, r8, c3.x
    mad_pp r7.xyz, r2.x, r8, r7
    add r3.w, -c52.z, c52.w
    rcp r3.w, r3.w
    add r8.xyz, -c50, v2
    dp3 r4.w, r8, r8
    rsq r4.w, r4.w
    rcp r5.w, r4.w
    mul_pp r8.xyz, r4.w, r8
    add r4.w, r5.w, -c52.z
    mul_sat r3.w, r3.w, r4.w
    add_pp r3.w, -r3.w, c3.y
    dp3_pp r4.w, r6, -r8
    mul_pp r9.xyz, r4.w, c51
    cmp_pp r9.xyz, r4.w, r9, c3.x
    mad_pp r7.xyz, r3.w, r9, r7
    dp3_pp r4.w, r6, -c10
    mul r9.xyz, r4.w, c36
    mul_pp r9.xyz, r1.x, r9
    cmp_pp r9.xyz, r4.w, r9, c3.x
    add_pp r7.xyz, r7, r9
    mul_pp r9.xyz, c58.w, c58
    dp3_pp r4.w, r6, -r1.yzww
    mul_pp r9.xyz, r4.w, r9
    cmp_pp r9.xyz, r4.w, r9, c3.x
    mad_pp r7.xyz, r0.w, r9, r7
    add_pp r0.xyz, r0, r7
    mul_pp r0.xyz, r0, c16
    frc r7.xy, c26.xwzw
    cmp r7.zw, -r7.xyxy, c3.x, c3.y
    add r7.xy, -r7, c26.xwzw
    cmp r7.zw, c26.xyxw, r3.x, r7
    add r7.xy, r7.zwzw, r7
    add r9, r7.y, -c3
    add r7, r7.x, -c3
    cmp r10.xy, -r9_abs.x, v0, c3.x
    cmp r9.xy, -r9_abs.y, v0.zwzw, r10
    cmp r9.xy, -r9_abs.z, v1, r9
    cmp r9.xy, -r9_abs.w, v1.zwzw, r9
    texld_pp r9, r9, s2
    mul_pp r9.xy, r9.x, c20.yzzw
    mul_pp r9.xy, r9, c2
    mul_pp r10.xyz, r9.y, c43
    max_pp r3.x, r9.x, c3.y
    min r4.w, r3.x, c2.z
    add_pp r9.xzw, c22.xyyz, -v2.xyyz
    dp3_pp r3.x, r9.xzww, r9.xzww
    rsq_pp r3.x, r3.x
    mad_pp r5.xyz, r9.xzww, r3.x, -r5
    nrm_pp r11.xyz, r5
    dp3_sat_pp r5.x, r11, r6
    pow r6.w, r5.x, r4.w
    mul_pp r5.xyz, r6.w, r10
    mul_pp r5.xyz, r3.z, r5
    mul_pp r10.xyz, r9.y, c39
    mad_pp r4.xyz, r9.xzww, r3.x, -r4
    nrm_pp r11.xyz, r4
    dp3_sat_pp r3.z, r11, r6
    pow r5.w, r3.z, r4.w
    mul_pp r4.xyz, r5.w, r10
    mad_pp r4.xyz, r3.y, r4, r5
    mul_pp r5.xyz, r9.y, c47
    mad_pp r2.yzw, r9.xxzw, r3.x, -r2
    nrm_pp r10.xyz, r2.yzww
    dp3_sat_pp r2.y, r10, r6
    pow r3.y, r2.y, r4.w
    mul_pp r2.yzw, r3.y, r5.xxyz
    mad_pp r2.xyz, r2.x, r2.yzww, r4
    mul_pp r4.xyz, r9.y, c51
    mad_pp r5.xyz, r9.xzww, r3.x, -r8
    nrm_pp r8.xyz, r5
    dp3_sat_pp r2.w, r8, r6
    pow r3.y, r2.w, r4.w
    mul_pp r4.xyz, r3.y, r4
    mad_pp r2.xyz, r3.w, r4, r2
    mul_pp r3.yzw, r9.y, c37.xxyz
    mad_pp r4.xyz, r9.xzww, r3.x, -c10
    nrm_pp r5.xyz, r4
    dp3_sat_pp r2.w, r5, r6
    pow r5.x, r2.w, r4.w
    mul_pp r3.yzw, r3, r5.x
    mad_pp r2.xyz, r3.yzww, r1.x, r2
    mul_pp r3.yzw, c59.w, c59.xxyz
    mul_pp r3.yzw, r9.y, r3
    mad_pp r1.xyz, r9.xzww, r3.x, -r1.yzww
    mul_pp r4.xyz, r3.x, r9.xzww
    dp3_sat_pp r1.w, r4, r6
    rsq_pp r1.w, r1.w
    rcp_pp r1.w, r1.w
    add r1.w, -r1.w, c3.y
    mul r4.xyz, r1.w, c88
    nrm_pp r5.xyz, r1
    dp3_sat_pp r1.x, r5, r6
    pow r2.w, r1.x, r4.w
    mul_pp r1.xyz, r2.w, r3.yzww
    mad_pp r1.xyz, r0.w, r1, r2
    mul_pp r1.xyz, r1, c18
    mul_pp r1.xyz, r1, v3.w
    cmp r2.xy, -r7_abs.x, v0, c3.x
    cmp r2.xy, -r7_abs.y, v0.zwzw, r2
    cmp r2.xy, -r7_abs.z, v1, r2
    cmp r2.xy, -r7_abs.w, v1.zwzw, r2
    texld_pp r2, r2, s0
    mul_pp r0.w, r2.w, c16.w
    mul_pp r0.w, r0.w, v6.w
    mul_pp r3.w, r0.w, c21.x
    add r0.w, c0.x, vPos.y
    mov r5.x, c86.x
    mad r0.w, r0.w, c24.w, -r5.x
    mul_sat_pp r0.w, r0.w, c86.y
    lrp_pp r5.xyz, r0.w, r2, c3.y
    mad_pp r0.xyz, r0, r5, r1
    mad_pp r3.xyz, r0, v6, r4
    mul_pp r0, r3, c89
    min_pp r1, r0, c2.w
    mul_pp r0.xyz, c75, v5.y
    mad_pp oC0.xyz, r1, v5.x, r0
    mov_pp oC0.w, r1.w

// approximately 235 instruction slots used (4 texture, 231 arithmetic)
