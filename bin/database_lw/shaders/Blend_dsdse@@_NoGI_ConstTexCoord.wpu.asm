//
// Generated by Microsoft (R) HLSL Shader Compiler 9.29.952.3111
//
// Parameters:
//
//   float4 g_Ambient;
//   float4 g_Diffuse;
//   float4 g_EyePosition;
//   float4 g_PowerGlossLevel;
//   float4 g_Specular;
//   float4 g_aLightField[6];
//   float4 mrgGlobalLight_Diffuse;
//   float4 mrgGlobalLight_Direction;
//   float4 mrgGlobalLight_Specular;
//   float4 mrgInShadowScale;
//   float4 mrgLocalLight1_Color;
//   float4 mrgLocalLight1_Position;
//   float4 mrgLocalLight1_Range;
//   float4 mrgLocalLight2_Color;
//   float4 mrgLocalLight2_Position;
//   float4 mrgLocalLight2_Range;
//   float4 mrgLocalLight3_Color;
//   float4 mrgLocalLight3_Position;
//   float4 mrgLocalLight3_Range;
//   float4 mrgLocalLight4_Color;
//   float4 mrgLocalLight4_Position;
//   float4 mrgLocalLight4_Range;
//   float4 mrgLuminanceRange;
//   sampler2D sampDLScatter;
//   sampler2D sampDif0;
//   sampler2D sampDif1;
//   sampler2D sampEnv;
//   sampler2D sampSpe0;
//   sampler2D sampSpe1;
//
//
// Registers:
//
//   Name                     Reg   Size
//   ------------------------ ----- ----
//   mrgGlobalLight_Direction c10      1
//   g_Diffuse                c16      1
//   g_Ambient                c17      1
//   g_Specular               c18      1
//   g_PowerGlossLevel        c20      1
//   g_EyePosition            c22      1
//   mrgGlobalLight_Diffuse   c36      1
//   mrgGlobalLight_Specular  c37      1
//   mrgLocalLight1_Position  c42      1
//   mrgLocalLight1_Color     c43      1
//   mrgLocalLight1_Range     c44      1
//   mrgLocalLight2_Position  c46      1
//   mrgLocalLight2_Color     c47      1
//   mrgLocalLight2_Range     c48      1
//   mrgLocalLight3_Position  c50      1
//   mrgLocalLight3_Color     c51      1
//   mrgLocalLight3_Range     c52      1
//   mrgLocalLight4_Position  c54      1
//   mrgLocalLight4_Color     c55      1
//   mrgLocalLight4_Range     c56      1
//   mrgLuminanceRange        c63      1
//   mrgInShadowScale         c64      1
//   g_aLightField            c77      6
//   sampDif0                 s0       1
//   sampDif1                 s1       1
//   sampSpe0                 s2       1
//   sampSpe1                 s3       1
//   sampEnv                  s5       1
//   sampDLScatter            s9       1
//

    ps_3_0
    def c0, 0.5, 10, 4, 1
    def c1, 0.25, 0, 0, 0
    def c2, -1, 1, 0, 0.5
    dcl_texcoord v0.xy
    dcl_texcoord1 v1.xy
    dcl_texcoord2 v2.xyz
    dcl_texcoord3_pp v3
    dcl_texcoord5 v4.xy
    dcl_color_pp v5
    dcl_2d s0
    dcl_2d s1
    dcl_2d s2
    dcl_2d s3
    dcl_2d s5
    dcl_2d s9
    texld_pp r0, v0, s0
    texld_pp r1, v1, s1
    lrp_pp r2, v5.w, r0, r1
    mul_pp oC0.w, r2.w, c16.w
    add_pp r0.xyz, -c50, v2
    dp3_pp r0.w, r0, r0
    rsq_pp r0.w, r0.w
    rcp_pp r1.x, r0.w
    mul_pp r0.xyz, r0.w, r0
    mad_sat_pp r0.w, -r1.x, c52.z, c52.w
    mul r1.xyz, r0.w, c51
    add_pp r3.xyz, -c42, v2
    dp3_pp r0.w, r3, r3
    rsq_pp r0.w, r0.w
    rcp_pp r1.w, r0.w
    mul_pp r3.xyz, r0.w, r3
    mad_sat_pp r0.w, -r1.w, c44.z, c44.w
    mul r4.xyz, r0.w, c43
    add_pp r5.xyz, -c46, v2
    dp3_pp r0.w, r5, r5
    rsq_pp r0.w, r0.w
    rcp_pp r1.w, r0.w
    mul_pp r5.xyz, r0.w, r5
    mad_sat_pp r0.w, -r1.w, c48.z, c48.w
    mul r6.xyz, r0.w, c47
    mov r7.y, c0.y
    mul_pp r0.w, r7.y, c20.y
    exp_pp r0.w, r0.w
    mul_pp r0.w, r0.w, c0.z
    add_pp r7, c22.xyzx, -v2.xyzx
    dp3_pp r1.w, r7.yzww, r7.yzww
    rsq_pp r1.w, r1.w
    mad_pp r8.xyz, r7, r1.w, -r5
    nrm_pp r9.xyz, r8
    nrm r8, v3.xyzx
    dp3_sat_pp r2.w, r9, r8
    pow_pp r3.w, r2.w, r0.w
    mul_pp r2.w, r3.w, c20.z
    mul_pp r9.xyz, r6, r2.w
    mad_pp r10.xyz, r7, r1.w, -r3
    dp3_sat_pp r2.w, r8.wyzw, -r3
    nrm_pp r3.xyz, r10
    dp3_sat_pp r3.x, r3, r8
    pow_pp r4.w, r3.x, r0.w
    mul_pp r3.x, r4.w, c20.z
    mad_pp r3.xyz, r3.x, r4, r9
    mad_pp r9.xyz, r7, r1.w, -r0
    dp3_sat_pp r0.x, r8.wyzw, -r0
    nrm_pp r10.xyz, r9
    dp3_sat_pp r0.y, r10, r8
    pow_pp r3.w, r0.y, r0.w
    mul_pp r0.y, r3.w, c20.z
    mad_pp r3.xyz, r0.y, r1, r3
    add_pp r9.xyz, -c54, v2
    dp3_pp r0.y, r9, r9
    rsq_pp r0.y, r0.y
    rcp_pp r0.z, r0.y
    mul_pp r9.xyz, r0.y, r9
    mad_sat_pp r0.y, -r0.z, c56.z, c56.w
    mul r10.xyz, r0.y, c55
    mad_pp r11.xyz, r7, r1.w, -r9
    dp3_sat_pp r0.y, r8.wyzw, -r9
    nrm_pp r9.xyz, r11
    dp3_sat_pp r0.z, r9, r8
    pow_pp r3.w, r0.z, r0.w
    mul_pp r0.z, r3.w, c20.z
    mad_pp r3.xyz, r0.z, r10, r3
    mad_pp r9.xyz, r7, r1.w, -c10
    mul r7, r1.w, r7
    nrm_pp r11.xyz, r9
    dp3_sat_pp r0.z, r11, r8
    pow_pp r1.w, r0.z, r0.w
    mov r9, c77
    mul_pp r0.z, r9.w, c20.z
    mul_pp r0.z, r1.w, r0.z
    mad_pp r3.xyz, r0.z, c37, r3
    dp3 r0.z, r7.yzww, r8.yzww
    add r0.z, r0.z, r0.z
    mad r7, r0.z, r8, -r7
    mul r11, r7.wyzw, c2.xxyz
    mad r12, r7, c2.yxxz, c2.zzzw
    cmp r7, r7.z, r11, r12
    add r0.z, r7.z, c0.w
    rcp r0.z, r0.z
    mad r0.zw, r7.xyyx, r0.z, c0.w
    mad_pp r7.x, r0.w, c1.x, r7.w
    mul_pp r7.y, r0.z, c0.x
    texld_pp r7, r7, s5
    mul r0.z, c64.z, c64.x
    mov r11.xz, c64
    mad r0.w, r11.x, -r11.z, r11.z
    mad_pp r0.z, c77.w, r0.w, r0.z
    texld_pp r11, v0, s2
    texld_pp r12, v1, s3
    lrp r13, v5.w, r11, r12
    mul r0.w, r13.w, c63.x
    mul r0.z, r0.z, r0.w
    mul_pp r7.xyz, r0.z, r7
    mad_pp r3.xyz, r3, c18, r7
    mul_pp r3.xyz, r13, r3
    mul_pp r3.xyz, r3, v3.w
    dp3_sat_pp r0.z, r8.wyzw, -r5
    mul_pp r5.xyz, r6, r0.z
    mad_pp r4.xyz, r2.w, r4, r5
    mad_pp r0.xzw, r0.x, r1.xyyz, r4.xyyz
    mad_pp r0.xyz, r0.y, r10, r0.xzww
    dp3_sat_pp r0.w, r8.wyzw, -c10
    mul r0.w, r0.w, c77.w
    mad_pp r0.xyz, r0.w, c36, r0
    mad_pp r1.xyz, r8.wyzw, c0.x, c0.x
    mul_pp r4.xyz, r8.wyzw, r8.wyzw
    mov r5.xyz, c80
    mad r5.xyz, r5, r1.y, c79
    mul r5.xyz, r4.y, r5
    mad r1.xyw, c78.xyzz, r1.x, r9.xyzz
    mov r6.xyz, c82
    mad r6.xyz, r6, r1.z, c81
    mad_pp r1.xyz, r4.x, r1.xyww, r5
    mad_pp r1.xyz, r4.z, r6, r1
    mad_pp r0.xyz, r1, c17, r0
    mul_pp r0.xyz, r0, c16
    mad_pp r0.xyz, r0, r2, r3
    mul_pp r0.xyz, r0, v5
    texld_pp r1, v4.yxzw, s9
    mad_pp oC0.xyz, r0, r1.w, r1

// approximately 147 instruction slots used (6 texture, 141 arithmetic)
