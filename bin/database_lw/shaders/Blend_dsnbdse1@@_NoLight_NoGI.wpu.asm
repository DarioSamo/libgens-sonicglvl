//
// Generated by Microsoft (R) HLSL Shader Compiler 9.29.952.3111
//
// Parameters:
//
//   float4 g_Ambient;
//   float4 g_Diffuse;
//   float4 g_EyePosition;
//   sampler2D g_FramebufferSampler;
//   float4 g_GIModeParam;
//   row_major float4x4 g_MtxView;
//   float4 g_PowerGlossLevel;
//   float4 g_Specular;
//   float4 g_ViewportSize;
//   float4 g_aLightField[6];
//   float4 mrgEyeLight_Diffuse;
//   float4 mrgEyeLight_Range;
//   float4 mrgEyeLight_Specular;
//   float4 mrgGlobalLight_Diffuse;
//   float4 mrgGlobalLight_Direction;
//   float4 mrgGlobalLight_Specular;
//   float4 mrgInShadowScale;
//   float4 mrgLuminanceRange;
//   float4 mrgTexcoordIndex[4];
//   sampler2D sampBlend;
//   sampler2D sampDLScatter;
//   sampler2D sampDif0;
//   sampler2D sampDif1;
//   sampler2D sampEnv;
//   sampler2D sampNrm;
//   sampler2D sampSpc0;
//
//
// Registers:
//
//   Name                     Reg   Size
//   ------------------------ ----- ----
//   mrgGlobalLight_Direction c10      1
//   g_Diffuse                c16      1
//   g_Ambient                c17      1
//   g_Specular               c18      1
//   g_PowerGlossLevel        c20      1
//   g_EyePosition            c22      1
//   g_ViewportSize           c24      1
//   mrgTexcoordIndex         c26      2
//   mrgGlobalLight_Diffuse   c36      1
//   mrgGlobalLight_Specular  c37      1
//   mrgEyeLight_Diffuse      c58      1
//   mrgEyeLight_Specular     c59      1
//   mrgEyeLight_Range        c60      1
//   mrgLuminanceRange        c63      1
//   mrgInShadowScale         c64      1
//   g_GIModeParam            c69      1
//   g_aLightField            c77      6
//   g_MtxView                c90      3
//   sampDif0                 s0       1
//   sampDif1                 s1       1
//   sampSpc0                 s2       1
//   sampNrm                  s4       1
//   sampBlend                s5       1
//   sampEnv                  s6       1
//   sampDLScatter            s9       1
//   g_FramebufferSampler     s11      1
//

    ps_3_0
    def c0, 4, 0.5, -0.5, 0
    def c1, 2, -1, 0.5, 10
    def c2, 0, 1, 2, 3
    dcl_texcoord v0
    dcl_texcoord1 v1
    dcl_texcoord2 v2.xyz
    dcl_texcoord3_pp v3
    dcl_texcoord5 v4.xy
    dcl_texcoord7 v5.xyz
    dcl_texcoord8 v6.xyz
    dcl_color_pp v7
    dcl vPos.xy
    dcl_2d s0
    dcl_2d s1
    dcl_2d s2
    dcl_2d s4
    dcl_2d s5
    dcl_2d s6
    dcl_2d s9
    dcl_2d s11
    mul r0.x, c64.z, c64.x
    mov r1.xz, c64
    mad r0.y, r1.x, -r1.z, r1.z
    mad_pp r0.x, c77.w, r0.y, r0.x
    frc r0.yzw, c26.xxyz
    cmp r1.xyz, -r0.yzww, c2.x, c2.y
    add r0.yzw, -r0, c26.xxyz
    mov r2.x, c2.x
    cmp r1.xyz, c26, r2.x, r1
    add r0.yzw, r0, r1.xxyz
    add r1, r0.w, -c2
    cmp r2.yz, -r1_abs.x, v0.xxyw, c2.x
    cmp r1.xy, -r1_abs.y, v0.zwzw, r2.yzzw
    cmp r1.xy, -r1_abs.z, v1, r1
    cmp r1.xy, -r1_abs.w, v1.zwzw, r1
    texld r1, r1, s2
    mul r0.w, r1.w, c63.x
    mul r0.x, r0.x, r0.w
    frc r2.yz, c27.xxyw
    cmp r3.xy, -r2.yzzw, c2.x, c2.y
    add r2.yz, -r2, c27.xxyw
    cmp r2.xw, c27.xyzy, r2.x, r3.xyzy
    add r2.xy, r2.xwzw, r2.yzzw
    add r3, r2.x, -c2
    add r2, r2.y, -c2
    cmp r4.xy, -r3_abs.x, v0, c2.x
    cmp r3.xy, -r3_abs.y, v0.zwzw, r4
    cmp r3.xy, -r3_abs.z, v1, r3
    cmp r3.xy, -r3_abs.w, v1.zwzw, r3
    texld_pp r3, r3, s4
    mul_pp r3.x, r3.w, r3.x
    mad_pp r3.xy, r3, c1.x, c1.y
    nrm_pp r4.xyz, v6
    mul_pp r4.xyz, r3.y, r4
    nrm_pp r5.xyz, v5
    mad_pp r4.xyz, r3.x, r5, r4
    dp2add_sat_pp r0.w, r3, r3, c2.x
    add r0.w, -r0.w, c2.y
    rsq r0.w, r0.w
    rcp_pp r0.w, r0.w
    nrm_pp r3.xyz, v3
    mad_pp r3.xyz, r0.w, r3, r4
    mul r4, r3.y, c91
    mad r4, r3.x, c90, r4
    mad r4, r3.z, c92, r4
    dp4 r0.w, r4, r4
    rsq r0.w, r0.w
    mul r4.xy, r0.w, r4
    mad_pp r4.xy, r4, c0.yzzw, c0.y
    texld_pp r4, r4, s6
    mul_pp r4.xyz, r0.x, r4
    add r5.xyz, c22, -v2
    dp3 r0.x, r5, r5
    rsq r0.x, r0.x
    mad_pp r6.xyz, r5, r0.x, -c10
    mul_pp r5.xyz, r0.x, r5
    rcp r0.x, r0.x
    mad_sat_pp r0.x, -r0.x, c60.z, c60.w
    nrm_pp r7.xyz, r6
    dp3_sat_pp r0.w, r7, r3
    mov r1.w, c1.w
    mul_pp r1.w, r1.w, c20.y
    exp_pp r1.w, r1.w
    mul_pp r1.w, r1.w, c0.x
    pow_pp r3.w, r0.w, r1.w
    mov r6, c77
    mul_pp r0.w, r6.w, c20.z
    mul_pp r0.w, r3.w, r0.w
    add_pp r7.xyz, r5, r5
    dp3_sat_pp r3.w, r3, r5
    nrm_pp r5.xyz, r7
    dp3_sat_pp r4.w, r5, r3
    pow_pp r5.x, r4.w, r1.w
    mul_pp r1.w, r5.x, c20.z
    mul_pp r5.xyz, r0.x, c59
    mul r7.xyz, r0.x, c58
    mul_pp r7.xyz, r3.w, r7
    mul_pp r5.xyz, r1.w, r5
    mad_pp r5.xyz, r0.w, c37, r5
    mad_pp r4.xyz, r5, c18, r4
    mul_pp r1.xyz, r1, r4
    mul_pp r1.xyz, r1, v3.w
    cmp r0.xw, -r2_abs.x, v0.xyzy, c2.x
    cmp r0.xw, -r2_abs.y, v0.zyzw, r0
    cmp r0.xw, -r2_abs.z, v1.xyzy, r0
    cmp r0.xw, -r2_abs.w, v1.zyzw, r0
    texld_pp r2, r0.xwzw, s5
    add r4, r0.y, -c2
    add r0, r0.z, -c2
    cmp r2.xz, -r4_abs.x, v0.xyyw, c2.x
    cmp r2.xz, -r4_abs.y, v0.zyww, r2
    cmp r2.xz, -r4_abs.z, v1.xyyw, r2
    cmp r2.xz, -r4_abs.w, v1.zyww, r2
    texld_pp r4, r2.xzzw, s0
    cmp r2.xz, -r0_abs.x, v0.xyyw, c2.x
    cmp r0.xy, -r0_abs.y, v0.zwzw, r2.xzzw
    cmp r0.xy, -r0_abs.z, v1, r0
    cmp r0.xy, -r0_abs.w, v1.zwzw, r0
    texld_pp r0, r0, s1
    lrp_pp r5, r2.y, r4, r0
    mad_pp r0.xyz, r3, c1.z, c1.z
    mov r2.xyz, c80
    mad r2.xyz, r2, r0.y, c79
    mul_pp r4.xyz, r3, r3
    dp3_sat_pp r0.y, r3, -c10
    mul r0.y, r0.y, c77.w
    mad_pp r3.xyz, r0.y, c36, r7
    mul r2.xyz, r2, r4.y
    mad r0.xyw, c78.xyzz, r0.x, r6.xyzz
    mov r6.xyz, c82
    mad r6.xyz, r6, r0.z, c81
    mad_pp r0.xyz, r4.x, r0.xyww, r2
    mad_pp r0.xyz, r4.z, r6, r0
    mad_pp r0.xyz, r0, c17, r3
    mul_pp r0.xyz, r0, c16
    mad_pp r0.xyz, r0, r5, r1
    mul_pp r0.w, r5.w, c16.w
    mul_pp r0, r0, v7
    texld_pp r1, v4.yxzw, s9
    mad_pp r0.xyz, r0, r1.w, r1
    mov_pp oC0.w, r0.w
    add r1.xy, c1.z, vPos
    mul r1.xy, r1, c24.zwzw
    texld r1, r1, s11
    add r1.xyz, -r0, r1
    mul r0.w, c69.z, v4.x
    mad_pp oC0.xyz, r0.w, r1, r0

// approximately 142 instruction slots used (8 texture, 134 arithmetic)
