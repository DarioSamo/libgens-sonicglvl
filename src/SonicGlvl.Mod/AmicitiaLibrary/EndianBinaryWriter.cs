...
namespace AmicitiaLibrary.IO
{
    public class EndianBinaryWriter : BinaryWriter
    {
        internal class ScheduledWrite
        {
            ...
            public ScheduledWrite(long position, Func<long> action, int priority, int sub = 0)
            {
                Position = position;
                Action = action;
                Priority = priority;
        ...
        public void ScheduleOffsetWrite(int priority, Action action, int sub = 0)
        {
        ...
        public void ScheduleOffsetWriteAligned(int alignment, Action action, int sub = 0)
        {
            ScheduleOffsetWrite(0, () =>
            {
        ...
        private void ScheduleOffsetWrite(int priority, Func<long> action, int sub = 0)
        {
            mScheduledWrites.AddLast(new ScheduledWrite(BaseStream.Position, action, priority, sub));
            Write(0);
        }
        ...
        private void DoScheduledWrite(ScheduledWrite scheduledWrite)
        {
            ...
            // Do actual write
            long offset = (scheduledWrite.Action() - mBaseOffset.Peek()) - scheduledWrite.Sub;
