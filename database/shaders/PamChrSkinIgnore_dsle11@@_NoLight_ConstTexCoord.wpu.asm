//
// Generated by Microsoft (R) HLSL Shader Compiler 9.29.952.3111
//
// Parameters:
//
//   float4 g_Ambient;
//   float4 g_ChrPlayableMenuParam;
//   float4 g_Diffuse;
//   float4 g_EyeDirection;
//   float4 g_EyePosition;
//   float4 g_ForceAlphaColor;
//   float4 g_GI0Scale;
//   float4 g_LightScatteringColor;
//   row_major float4x4 g_MtxView;
//   float4 g_OpacityReflectionRefractionSpectype;
//   float4 g_PowerGlossLevel;
//   float4 g_ShadowMapParams;
//   sampler2D g_ShadowMapSampler;
//   float4 g_SonicSkinFalloffParam;
//   float4 g_Specular;
//   sampler2D g_VerticalShadowMapSampler;
//   float4 g_aLightField[6];
//   float4 mrgChrEmissionParam;
//   float4 mrgEyeLight_Attribute;
//   float4 mrgEyeLight_Range;
//   float4 mrgEyeLight_Specular;
//   float4 mrgGlobalLight_Direction;
//   float4 mrgGlobalLight_Specular;
//   sampler2D sampDif;
//   sampler2D sampEmi;
//   sampler2D sampEnv;
//   sampler2D sampSpec;
//
//
// Registers:
//
//   Name                                  Reg   Size
//   ------------------------------------- ----- ----
//   mrgGlobalLight_Direction              c10      1
//   g_Diffuse                             c16      1
//   g_Ambient                             c17      1
//   g_Specular                            c18      1
//   g_PowerGlossLevel                     c20      1
//   g_OpacityReflectionRefractionSpectype c21      1
//   g_EyePosition                         c22      1
//   g_EyeDirection                        c23      1
//   mrgGlobalLight_Specular               c37      1
//   mrgEyeLight_Specular                  c59      1
//   mrgEyeLight_Range                     c60      1
//   mrgEyeLight_Attribute                 c61      1
//   g_ShadowMapParams                     c65      1
//   g_GI0Scale                            c70      1
//   g_LightScatteringColor                c75      1
//   g_aLightField                         c77      6
//   g_ForceAlphaColor                     c89      1
//   g_MtxView                             c90      3
//   g_SonicSkinFalloffParam               c150     1
//   mrgChrEmissionParam                   c151     1
//   g_ChrPlayableMenuParam                c152     1
//   sampDif                               s0       1
//   sampSpec                              s1       1
//   sampEmi                               s2       1
//   sampEnv                               s3       1
//   g_VerticalShadowMapSampler            s7       1
//   g_ShadowMapSampler                    s13      1
//

    ps_3_0
    def c0, 0.0500000007, 10, 500, 5
    def c1, 0.5, 0, 1, -2
    def c2, 1024, -2, 3, 4
    def c3, 0.5, -0.5, 0, 0
    dcl_texcoord v0.xy
    dcl_texcoord2 v1.xyz
    dcl_texcoord3_pp v2
    dcl_texcoord4 v3
    dcl_texcoord6 v4
    dcl_texcoord5_pp v5.xyz
    dcl_color_pp v6
    dcl_2d s0
    dcl_2d s1
    dcl_2d s2
    dcl_2d s3
    dcl_2d s7
    dcl_2d s13
    max r0.x, v3.z, c1.y
    add r0.y, r0.x, -v3.w
    cmp r0.z, r0.y, c1.y, r0.x
    mov r0.xyw, v3
    texldp_pp r0, r0, s13
    max r0.y, v4.z, c1.y
    add r0.z, r0.y, -v4.w
    cmp r1.z, r0.z, c1.y, r0.y
    mov r1.xyw, v4
    texldp_pp r1, r1, s7
    min_pp r2.x, r1.x, r0.x
    lrp_pp r0.x, v5.z, c1.z, r1.x
    lrp_pp r0.y, v5.z, c1.z, r2.x
    rcp r0.z, v3.w
    mul r0.zw, r0.z, v3.xyxy
    mad r0.zw, r0, -c1.w, -c1.z
    abs_sat r0.zw, r0
    add_sat r0.zw, r0, -c65.x
    mul r0.zw, r0, c65.y
    max r1.x, r0.z, r0.w
    lrp_pp r2.x, r1.x, c1.z, r0.y
    lrp_pp r2.y, r1.x, c1.z, r0.x
    mul_pp r0.x, r2.x, c77.w
    add_pp r0.y, -c61.z, c61.y
    rcp_pp r0.y, r0.y
    add r1.xyz, -c22, v1
    dp3 r0.z, r1, r1
    rsq r0.z, r0.z
    mul_pp r1.xyz, r0.z, r1
    rcp r0.z, r0.z
    add r0.z, r0.z, -c60.z
    dp3_pp r0.w, r1, c23
    add_pp r0.w, r0.w, -c61.z
    mul_sat_pp r0.y, r0.y, r0.w
    mad_pp r0.w, r0.y, c2.y, c2.z
    mul_pp r0.y, r0.y, r0.y
    mul_pp r0.y, r0.y, r0.w
    pow_sat_pp r1.w, r0.y, c61.w
    mov_pp r2.z, c1.z
    add r0.y, r2.z, -c61.x
    cmp_pp r0.y, r0.y, c1.z, r1.w
    add r0.w, -c60.z, c60.w
    rcp r0.w, r0.w
    mul_sat r0.z, r0.w, r0.z
    add_pp r0.z, -r0.z, c1.z
    cmp_pp r0.z, -c61.x, r2.z, r0.z
    mul_pp r0.y, r0.y, r0.z
    mul_pp r2.xzw, c59.w, c59.xyyz
    mov_pp r0.zw, c0
    mul_pp r0.zw, r0, c20.xyyz
    mul_pp r2.xzw, r0.w, r2
    max_pp r1.w, r0.z, c1.z
    mul_pp r3.xyz, r0.w, c37
    min r0.z, r1.w, c2.x
    add_pp r4.xyz, c22, -v1
    dp3_pp r0.w, r4, r4
    rsq_pp r0.w, r0.w
    mad_pp r1.xyz, r4, r0.w, -r1
    nrm_pp r5.xyz, r1
    nrm_pp r1.xyz, v2
    dp3_sat_pp r1.w, r5, r1
    pow r3.w, r1.w, r0.z
    mul_pp r2.xzw, r2, r3.w
    mul_pp r2.xzw, r0.y, r2
    mad_pp r5.xyz, r4, r0.w, -c10
    mul_pp r4.xyz, r0.w, r4
    nrm_pp r6.xyz, r5
    dp3_sat_pp r0.y, r6, r1
    pow r1.w, r0.y, r0.z
    mul_pp r0.yzw, r1.w, r3.xxyz
    mad_pp r0.yzw, r0, r0.x, r2.xxzw
    mad r0.x, r0.x, c1.x, c1.x
    mul_pp r0.yzw, r0, c18.xxyz
    add_pp r1.w, c152.x, -v1.y
    add_pp r1.w, r1.w, c0.x
    mul_sat_pp r1.w, r1.w, c0.y
    mul_pp r0.yzw, r0, r1.w
    texld_pp r3, v0, s1
    mul_pp r0.yzw, r0, r3.xxyz
    mul_pp r0.yzw, r0, v2.w
    mov r3.xyz, c79
    add r2.xzw, -r3.xyyz, c80.xyyz
    mad_pp r3.xyz, r1, c1.x, c1.x
    mad r5.xyz, r3.y, r2.xzww, c79
    mul_pp r6.xyz, r1, r1
    mul r5.xyz, r5, r6.y
    mov r7.xyz, c77
    add r7.xyz, -r7, c78
    mad r3.xyw, r3.x, r7.xyzz, c77.xyzz
    mad_pp r3.xyw, r6.x, r3, r5.xyzz
    mov r5.xyz, c81
    add r5.xyz, -r5, c82
    mad r6.xyw, r3.z, r5.xyzz, c81.xyzz
    mad_pp r3.xyz, r6.z, r6.xyww, r3.xyww
    mul_pp r6.xyz, r3, c17
    mul r6.xyz, r6, c70.w
    mad r3.xyz, r3, c17, -r6
    mad_pp r3.xyz, r2.y, r3, r6
    add_pp r3.xyz, r3, c1.z
    mul_pp r3.xyz, r3, c16
    texld_pp r6, v0, s0
    lrp_pp r8, r1.w, r6, c1.z
    mul_pp r3.xyz, r3, r8
    mul_pp r6.w, r8.w, c16.w
    mul r8, r1.y, c91
    mad r8, r1.x, c90, r8
    mad r8, r1.z, c92, r8
    dp4 r2.y, r8, r8
    rsq r2.y, r2.y
    mul r8.xy, r2.y, r8
    mad_pp r8.xy, r8, c3, c3.x
    texld_pp r8, r8, s3
    mul_pp r8.xyz, r1.w, r8
    mad_pp r0.yzw, r3.xxyz, r8.xxyz, r0
    dp3_pp r2.y, r4, r1
    add_sat_pp r3.x, -r2.y, c1.z
    add_pp r2.y, r2.y, r2.y
    mad_pp r1.xyz, r2.y, r1, -r4
    pow_pp r2.y, r3.x, c150.z
    mul r2.y, r2.y, c150.y
    mad_pp r0.x, r2.y, r0.x, c150.x
    mad_pp r3.xyz, r1, c1.x, c1.x
    mul_pp r1.xyz, r1, r1
    mad r2.xyz, r3.y, r2.xzww, c79
    mul r2.xyz, r1.y, r2
    mad r3.xyw, r3.x, r7.xyzz, c77.xyzz
    mad r4.xyz, r3.z, r5, c81
    mad_pp r2.xyz, r1.x, r3.xyww, r2
    mad_pp r1.xyz, r1.z, r4, r2
    mul_pp r1.xyz, r1.w, r1
    mad_pp r0.xyz, r1, r0.x, r0.yzww
    texld r1, v0, s2
    add_pp r1.xyz, r1, c151
    mul_pp r1.xyz, r1, c17
    mad_pp r6.xyz, r1, c151.w, r0
    mul_pp r0, r6, v6
    mul_pp r0.w, r0.w, c21.x
    mul_pp r0, r0, c89
    min_pp r1, r0, c2.w
    mul_pp r0.xyz, c75, v5.y
    mad_pp oC0.xyz, r1, v5.x, r0
    mov_pp oC0.w, r1.w

// approximately 156 instruction slots used (6 texture, 150 arithmetic)
