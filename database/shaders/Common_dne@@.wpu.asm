//
// Generated by Microsoft (R) HLSL Shader Compiler 9.29.952.3111
//
// Parameters:
//
//   float4 g_Ambient;
//   float4 g_Diffuse;
//   float4 g_EyeDirection;
//   float4 g_EyePosition;
//   float4 g_ForceAlphaColor;
//   float4 g_GI0Scale;
//   float4 g_GI1Scale;
//   sampler2D g_GISampler;
//   float4 g_LightScatteringColor;
//   float4 g_OpacityReflectionRefractionSpectype;
//   float4 g_ShadowMapParams;
//   sampler2D g_ShadowMapSampler;
//   float4 g_ViewportSize;
//   float4 mrgEyeLight_Attribute;
//   float4 mrgEyeLight_Diffuse;
//   float4 mrgEyeLight_Range;
//   float4 mrgGlobalLight_Diffuse;
//   float4 mrgGlobalLight_Direction;
//   float4 mrgInShadowScale;
//   float4 mrgLocalLight0_Color;
//   float4 mrgLocalLight0_Position;
//   float4 mrgLocalLight0_Range;
//   float4 mrgLocalLight1_Color;
//   float4 mrgLocalLight1_Position;
//   float4 mrgLocalLight1_Range;
//   float4 mrgLocalLight2_Color;
//   float4 mrgLocalLight2_Position;
//   float4 mrgLocalLight2_Range;
//   float4 mrgLocalLight3_Color;
//   float4 mrgLocalLight3_Position;
//   float4 mrgLocalLight3_Range;
//   float4 mrgLuminanceRange;
//   float4 mrgPlayableParam;
//   float4 mrgTexcoordIndex[4];
//   sampler2D sampDif;
//   sampler2D sampEnv;
//   sampler2D sampNrm;
//
//
// Registers:
//
//   Name                                  Reg   Size
//   ------------------------------------- ----- ----
//   mrgGlobalLight_Direction              c10      1
//   g_Diffuse                             c16      1
//   g_Ambient                             c17      1
//   g_OpacityReflectionRefractionSpectype c21      1
//   g_EyePosition                         c22      1
//   g_EyeDirection                        c23      1
//   g_ViewportSize                        c24      1
//   mrgTexcoordIndex                      c26      2
//   mrgGlobalLight_Diffuse                c36      1
//   mrgLocalLight0_Position               c38      1
//   mrgLocalLight0_Color                  c39      1
//   mrgLocalLight0_Range                  c40      1
//   mrgLocalLight1_Position               c42      1
//   mrgLocalLight1_Color                  c43      1
//   mrgLocalLight1_Range                  c44      1
//   mrgLocalLight2_Position               c46      1
//   mrgLocalLight2_Color                  c47      1
//   mrgLocalLight2_Range                  c48      1
//   mrgLocalLight3_Position               c50      1
//   mrgLocalLight3_Color                  c51      1
//   mrgLocalLight3_Range                  c52      1
//   mrgEyeLight_Diffuse                   c58      1
//   mrgEyeLight_Range                     c60      1
//   mrgEyeLight_Attribute                 c61      1
//   mrgLuminanceRange                     c63      1
//   mrgInShadowScale                      c64      1
//   g_ShadowMapParams                     c65      1
//   g_GI0Scale                            c70      1
//   g_GI1Scale                            c71      1
//   g_LightScatteringColor                c75      1
//   mrgPlayableParam                      c86      1
//   g_ForceAlphaColor                     c89      1
//   sampDif                               s0       1
//   sampNrm                               s3       1
//   sampEnv                               s5       1
//   g_GISampler                           s10      1
//   g_ShadowMapSampler                    s13      1
//

    ps_3_0
    def c0, 0.298900008, 0.586600006, 0.114399999, 0.25
    def c1, 4, 0, 0, 0
    def c2, 0.5, 2, -1, -3
    def c3, 0, 1, 2, 3
    def c4, -1, 1, 0, 0.5
    dcl_texcoord v0
    dcl_texcoord1 v1
    dcl_texcoord2 v2.xyz
    dcl_texcoord3_pp v3
    dcl_texcoord4 v4
    dcl_texcoord5_pp v5.xyz
    dcl_texcoord7 v6.xyz
    dcl_texcoord8 v7.xyz
    dcl_color_pp v8
    dcl vPos.y
    dcl_2d s0
    dcl_2d s3
    dcl_2d s5
    dcl_2d s10
    dcl_2d s13
    frc r0.x, c27.x
    cmp r0.y, -r0.x, c3.x, c3.y
    add r0.x, -r0.x, c27.x
    mov r1.xy, c3
    cmp r0.y, c27.x, r1.x, r0.y
    add r0.x, r0.y, r0.x
    add r0, r0.x, -c3
    cmp r1.zw, -r0_abs.x, v0.xyxy, c3.x
    cmp r0.xy, -r0_abs.y, v0.zwzw, r1.zwzw
    cmp r0.xy, -r0_abs.z, v1, r0
    cmp r0.xy, -r0_abs.w, v1.zwzw, r0
    texld_pp r0, r0, s3
    mul_pp r0.x, r0.w, r0.x
    mad_pp r0.xy, r0, c2.y, c2.z
    nrm_pp r2, v7.xyzx
    mul_pp r2, r0.y, r2
    nrm_pp r3, v6.xyzx
    mad_pp r2, r0.x, r3, r2
    dp2add_pp r0.x, r0, -r0, c3.y
    rsq_pp r0.x, r0.x
    rcp_pp r0.x, r0.x
    nrm_pp r3, v3.xyzx
    mad r0, r0.x, r3, r2
    add r2.xyz, -c42, v2
    dp3 r1.z, r2, r2
    rsq r1.z, r1.z
    mul_pp r2.xyz, r1.z, r2
    rcp r1.z, r1.z
    add r1.z, r1.z, -c44.z
    dp3_pp r1.w, r0.wyzw, -r2
    mul_pp r2.xyz, r1.w, c43
    cmp_pp r2.xyz, r1.w, r2, c3.x
    add r1.w, -c44.z, c44.w
    rcp r1.w, r1.w
    mul_sat r1.z, r1.w, r1.z
    add_pp r1.z, -r1.z, c3.y
    mul_pp r2.xyz, r2, r1.z
    add r3.xyz, -c38, v2
    dp3 r1.z, r3, r3
    rsq r1.z, r1.z
    mul_pp r3.xyz, r1.z, r3
    rcp r1.z, r1.z
    add r1.z, r1.z, -c40.z
    dp3_pp r1.w, r0.wyzw, -r3
    mul_pp r3.xyz, r1.w, c39
    cmp_pp r3.xyz, r1.w, r3, c3.x
    add r1.w, -c40.z, c40.w
    rcp r1.w, r1.w
    mul_sat r1.z, r1.w, r1.z
    add_pp r1.z, -r1.z, c3.y
    mad_pp r2.xyz, r1.z, r3, r2
    add r3.xyz, -c46, v2
    dp3 r1.z, r3, r3
    rsq r1.z, r1.z
    mul_pp r3.xyz, r1.z, r3
    rcp r1.z, r1.z
    add r1.z, r1.z, -c48.z
    dp3_pp r1.w, r0.wyzw, -r3
    mul_pp r3.xyz, r1.w, c47
    cmp_pp r3.xyz, r1.w, r3, c3.x
    add r1.w, -c48.z, c48.w
    rcp r1.w, r1.w
    mul_sat r1.z, r1.w, r1.z
    add_pp r1.z, -r1.z, c3.y
    mad_pp r2.xyz, r1.z, r3, r2
    add r3.xyz, -c50, v2
    dp3 r1.z, r3, r3
    rsq r1.z, r1.z
    mul_pp r3.xyz, r1.z, r3
    rcp r1.z, r1.z
    add r1.z, r1.z, -c52.z
    dp3_pp r1.w, r0.wyzw, -r3
    mul_pp r3.xyz, r1.w, c51
    cmp_pp r3.xyz, r1.w, r3, c3.x
    add r1.w, -c52.z, c52.w
    rcp r1.w, r1.w
    mul_sat r1.z, r1.w, r1.z
    add_pp r1.z, -r1.z, c3.y
    mad_pp r2.xyz, r1.z, r3, r2
    dp3_pp r1.z, r0.wyzw, -c10
    mul r3.xyz, r1.z, c36
    rcp r1.w, v4.w
    mul r4.xy, r1.w, v4
    mad r4.xy, r4, c2.y, c2.z
    abs_sat r4.xy, r4
    add_sat r4.xy, r4, -c65.x
    mul r4.xy, r4, c65.y
    max r1.w, r4.x, r4.y
    max r2.w, v4.z, c3.x
    add r3.w, r2.w, -v4.w
    cmp r4.z, r3.w, c3.x, r2.w
    mov r4.xyw, v4
    texldp_pp r4, r4, s13
    lrp_pp r2.w, v5.z, c3.y, r4.x
    lrp_pp r3.w, r1.w, c3.y, r2.w
    texld_pp r4, v0.zwzw, s10
    mul r1.w, r3.w, r4.w
    mul_pp r3.xyz, r1.w, r3
    cmp_pp r3.xyz, r1.z, r3, c3.x
    add_pp r2.xyz, r2, r3
    add_pp r1.z, -c61.z, c61.y
    rcp_pp r1.z, r1.z
    add r3.xyz, -c22, v2
    dp3 r2.w, r3, r3
    rsq r2.w, r2.w
    mul_pp r3.xyz, r2.w, r3
    rcp r2.w, r2.w
    add r2.w, r2.w, -c60.z
    dp3_pp r4.w, r3, c23
    dp3_pp r3.x, r0.wyzw, -r3
    add_pp r3.y, r4.w, -c61.z
    mul_sat_pp r1.z, r1.z, r3.y
    mad_pp r3.y, r1.z, -c2.y, -c2.w
    mul_pp r1.z, r1.z, r1.z
    mul_pp r1.z, r1.z, r3.y
    pow_sat_pp r3.y, r1.z, c61.w
    add r1.z, r1.y, -c61.x
    cmp_pp r1.z, r1.z, c3.y, r3.y
    add r3.y, -c60.z, c60.w
    rcp r3.y, r3.y
    mul_sat r2.w, r2.w, r3.y
    add_pp r2.w, -r2.w, c3.y
    cmp_pp r1.y, -c61.x, r1.y, r2.w
    mul_pp r1.y, r1.z, r1.y
    mul_pp r5.xyz, c58.w, c58
    mul_pp r5.xyz, r3.x, r5
    cmp_pp r3.xyz, r3.x, r5, c3.x
    mad_pp r2.xyz, r1.y, r3, r2
    mul_pp r3.xyz, r4, r4
    dp3_pp r1.y, r3, c0
    mad_pp r3.xyz, r4, r4, -r1.y
    mad_sat_pp r3.xyz, r3, c71.w, r1.y
    mul_pp r3.xyz, r3, c70
    mul_pp r4.xyz, r3, c17
    mul r4.xyz, r4, c70.w
    mad r3.xyz, r3, c17, -r4
    mad_pp r3.xyz, r3.w, r3, r4
    add_pp r2.xyz, r2, r3
    mul_pp r2.xyz, r2, c16
    frc r1.y, c26.x
    cmp r1.z, -r1.y, c3.x, c3.y
    add r1.y, -r1.y, c26.x
    cmp r1.x, c26.x, r1.x, r1.z
    add r1.x, r1.x, r1.y
    add r3, r1.x, -c3
    cmp r1.xy, -r3_abs.x, v0, c3.x
    cmp r1.xy, -r3_abs.y, v0.zwzw, r1
    cmp r1.xy, -r3_abs.z, v1, r1
    cmp r1.xy, -r3_abs.w, v1.zwzw, r1
    texld_pp r3, r1, s0
    mul_pp r1.x, r3.w, c16.w
    mul_pp r1.x, r1.x, v8.w
    mul_pp r4.w, r1.x, c21.x
    add r1.x, c2.x, vPos.y
    mov r5.x, c86.x
    mad r1.x, r1.x, c24.w, -r5.x
    mul_sat_pp r1.x, r1.x, c86.y
    lrp_pp r5.xyz, r1.x, r3, c3.y
    add_pp r3, c22.xyzx, -v2.xyzx
    dp3_pp r1.x, r3.yzww, r3.yzww
    rsq_pp r1.x, r1.x
    mul r3, r1.x, r3
    dp3 r1.x, r3.yzww, r0.yzww
    add r1.x, r1.x, r1.x
    mad r0, r1.x, r0, -r3
    mad r3, r0.wyzw, c4.xxyz, c4.zzzw
    mul r6, r0, c4.yxxz
    cmp r0, r0.z, r3, r6
    add r0.z, r0.z, c3.y
    rcp r0.z, r0.z
    mad r0.xy, r0.yxzw, r0.z, c3.y
    mad_pp r1.x, r0.y, c0.w, r0.w
    mul_pp r1.y, r0.x, c2.x
    texld_pp r0, r1, s5
    mul r1.xyz, r0, c63.x
    mad_pp r0.xyz, r1, r0.w, r0
    mul r0.w, c64.z, c64.x
    mov r1.xz, c64
    mad r1.x, r1.x, -r1.z, r1.z
    mad_pp r0.w, r1.w, r1.x, r0.w
    mul_pp r0.xyz, r0.w, r0
    mul_pp r0.xyz, r0, v3.w
    mad_pp r0.xyz, r2, r5, r0
    mul_pp r4.xyz, r0, v8
    mul_pp r0, r4, c89
    min_pp r1, r0, c1.x
    mul_pp r0.xyz, c75, v5.y
    mad_pp oC0.xyz, r1, v5.x, r0
    mov_pp oC0.w, r1.w

// approximately 198 instruction slots used (5 texture, 193 arithmetic)
